...
export async function addProductToCart(productId: string): Promise<AddResult> {
  try {
    ...
    if (!process.env.NEXTAUTH_SECRET)
      return { success: false, message: 'server-misconfig' };
    ...
    if (!credentialsToken)
      return { success: false, message: 'no-credentials-token' };
    ...
    if (!res.ok)
      return {
        success: false,
        message: data?.message || `API ${res.status}`,
      };
...
export async function removeItemFromCart(id: string) {
  try {
    ...
    const count =
      Number(data.numOfCartItems) ||
      Number(data.data?.numOfCartItems) ||
      0;
